...
const AdminDashboard: React.FC = () => {
  ...
  const [updatingRoleRequestUserId, setUpdatingRoleRequestUserId] = useState<string | null>(null);
  const [roleRequestAction, setRoleRequestAction] = useState<'approve' | 'reject' | null>(null);
  ...
  const handleRoleRequestAction = async (userId: string, action: 'approve' | 'reject') => {
    try {
      setUpdatingRoleRequestUserId(userId);
      setRoleRequestAction(action);
      await api.put(`/admin/role-requests/${userId}`, { action });
      await fetchUsers(activeUserIdSearch || undefined);
      showAlert({
        variant: 'success',
        title: 'role request updated',
      ...
      showAlert({
        ...
        message: err.response?.data?.message || `failed to ${action} role request`,
      });
    } finally {
      setUpdatingRoleRequestUserId(null);
      setRoleRequestAction(null);
    }
  };
                    ...
                    <div className="flex flex-col justify-center gap-2">
                      {user.status !== 'approved' && (
                        <button
                          ...
                          className="bg-green-600 hover:bg-green-700 text-white font-black text-[10px] py-2 px-4 rounded-lg shadow-sm disabled:opacity-50 min-w-25 cursor-pointer uppercase tracking-widest transition-all"
                        >
                      ...
                      {user.status !== 'rejected' && (
                        <button
                          ...
                          disabled={updatingUserId === user._id}
                          className="bg-red-600 hover:bg-red-700 text-white font-black text-[10px] py-2 px-4 rounded-lg shadow-sm disabled:opacity-50 min-w-25 cursor-pointer uppercase tracking-widest transition-all"
                        >
                      ...
                      {user.roleRequest?.status === 'pending' && (
                        <>
                          <button
                            onClick={() => handleRoleRequestAction(user._id, 'approve')}
                            disabled={updatingRoleRequestUserId === user._id}
                            className="bg-blue-600 hover:bg-blue-700 text-white font-black text-[10px] py-2 px-4 rounded-lg shadow-sm disabled:opacity-50 min-w-25 cursor-pointer uppercase tracking-widest transition-all"
                          >
                            {updatingRoleRequestUserId === user._id && roleRequestAction === 'approve' ? 'Approving Role...' : 'Approve Role'}
                          </button>
                          <button
                            onClick={() => handleRoleRequestAction(user._id, 'reject')}
                            disabled={updatingRoleRequestUserId === user._id}
                            className="bg-orange-600 hover:bg-orange-700 text-white font-black text-[10px] py-2 px-4 rounded-lg shadow-sm disabled:opacity-50 min-w-25 cursor-pointer uppercase tracking-widest transition-all"
                          >
                            {updatingRoleRequestUserId === user._id && roleRequestAction === 'reject' ? 'Rejecting Role...' : 'Reject Role'}
                          </button>
                        </>
                      )}
                    </div>
